...
train_pca_cubic = cubic_features(train_pca)
test_pca_cubic = cubic_features(test_pca)
...
def run_softmax_on_MNIST_pca_cubic(temp_parameter=1):
    ...
    train_x, train_y, test_x, test_y = get_MNIST_data()
    theta, cost_function_history = softmax_regression(
        train_pca_cubic, train_y, temp_parameter, alpha=0.3, lambda_factor=1.0e-4, k=10, num_iterations=150)
    plot_cost_function_over_time(cost_function_history)
    test_error = compute_test_error(test_pca_cubic, test_y, theta, temp_parameter)
    return test_error


# TODO: Run run_softmax_on_MNIST_mod3(), report the error rate
print('softmax test_error_pca_cubic=', run_softmax_on_MNIST_pca_cubic(temp_parameter=1))
